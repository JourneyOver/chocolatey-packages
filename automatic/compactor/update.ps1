...
function global:au_GetLatest {
  ...
  $url32 = $release.latestStable.Assets | Where-Object { $_ -match 'Compactor-(\d+)\.(\d+)\.(\d+)\-i686.zip$' } | Select-Object -First 1
  $url64 = $release.latestStable.Assets | Where-Object { $_ -match 'Compactor-(\d+)\.(\d+)\.(\d+)\.zip$' } | Select-Object -First 1
